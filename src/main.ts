...
import { CacheEntry, WeatherResponse } from "./models/weather.js";
import { GeocodingResponse } from "./models/geocoding.js";
...
async function fetchWeatherCached(lat: number, lon: number): Promise<WeatherResponse> {
  const key = `${lat},${lon}`;
  const cached = weatherCache.get(key);
  if (cached && Date.now() - cached.cachedAt < TTL_MS) return cached.data; // Retourne les données en cache si elles sont encore valides
...
const blois: Location = { name: "Blois", lat: BLOIS_LAT, lon: BLOIS_LON };
