...
export function analyzeWeatherTrend(weatherCodes: number[]): string {
  if (weatherCodes.length < 2) return "Données insuffisantes";
  ...
  if (delta > SEUIL) return "Amélioration";
  if (delta < -SEUIL) return "Dégradation";
  return "Stable";
}