import type { HourlyWeather } from "../models/weather.js";
import { getWeatherEmoji } from "../utils/weatherInterpreter.js";

let weatherChart: any = null; // stocker instance du graphique

export function renderWeatherChart(canvasId: string, data: HourlyWeather, currentTime: string) {
    ...
    if (weatherChart) weatherChart.destroy(); // si weather contient un graphique actuel on supprime de la mémoire
        ...
        options: {
            ...
            scales: {
                y: {// axe gauche temp
                    ...
                    ticks: { color: '#aaa', callback: (v: any) => v + '°' } //callback pour unité à coté des chiffres
                },
                y1: { //axe droite pluie
                    type: 'linear',
                    position: 'right',
                    grid: { display: false },
                    beginAtZero: true,
                    ticks: { color: '#60a5fa', callback: (v: any) => v + 'mm' }
                },
